...
#define DEV_PATH "/dev/cosim_doe_chardev" STR(MINOR)
...
static int connect_remote(void)
{
    ...
    LOG("Connected to thin-server %s:%d", SERVER_IP, SERVER_PORT);
    return sock;
}
...
int main(void)
{
    ...
    while (1) {
        ...
        if (pfd.revents & POLLIN) {
            ...
            LOG("RESP: status=%d value=0x%x", op.status, op.value);
